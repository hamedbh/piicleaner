...
impl PatternRegistry {
    fn new() -> Self {
        ...
        patterns.insert("cash-amount", vec![
            r"(([£€$]{1}|GBP|USD|EUR)\s?)[\d,.]{2,}",
            r"[\d\,\.]{2,}(\s?(GBP|USD|EUR)){1}",
        ]);
        
        patterns.insert("address", vec![
            r"\d{1,3} \w{3,} (street|lane|road|close|avenue|drive|grove|mansions|way)",
        ]);
        
        patterns.insert("case-id", vec![
        ...
        patterns.insert("postcode", vec![
            r"\b[A-Z]{1,2}\d[A-Z\d]? ?\d[A-Z]{2}\b",
        ]);
        
        patterns.insert("tag", vec![
            r"<.*script.*>",
            r"<.*>",
        ]);
