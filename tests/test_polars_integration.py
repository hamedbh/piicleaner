...
class TestPolarsDataFrameMethods:
    ...
    def test_clean_dataframe_redact(self, cleaner, sample_df):
        ...
        result = cleaner.clean_dataframe(sample_df, "text", "redact", "cleaned_text")
    ...
    def test_clean_dataframe_replace(self, cleaner, sample_df):
        ...
        result = cleaner.clean_dataframe(sample_df, "text", "replace", "cleaned_text")
...
class TestPolarsNamespaceAPI:
    ...
    def test_namespace_detect_pii_with_cleaners(self, sample_df):
        ...
        result = sample_df.with_columns(
            pl.col("text").pii.detect_pii_with_cleaners(["email"]).alias("email_matches")
        )
    ...
    def test_namespace_with_null_values(self):
        ...
        assert detected[1] == []   # Null should return empty list
...
class TestPolarsEdgeCases:
    ...
    def test_empty_dataframe(self):
        ...
        assert detected.shape == (0, 4)  # Should have structure but no rows
    
    def test_large_dataframe(self):
        """Test with large DataFrame."""
        cleaner = Cleaner()
        large_data = ["Email: test@example.com"] * 1000 + ["No PII"] * 1000
        large_df = pl.DataFrame({"text": large_data})
        
        cleaned = cleaner.clean_dataframe(large_df, "text", "redact", "cleaned")
        detected = cleaner.detect_dataframe(large_df, "text")
        
        assert cleaned.shape == (2000, 2)
        assert detected.shape[0] >= 1000  # Should detect PII in first 1000 rows
        
        cleaned_texts = cleaned["cleaned"].to_list()
        # First 1000 should be cleaned, last 1000 unchanged
        assert "test@example.com" not in cleaned_texts[0]
        assert cleaned_texts[1999] == "No PII"
    ...
    def test_specific_cleaners_dataframe(self):
        ...
        cleaned = email_cleaner.clean_dataframe(df, "text", "redact", "cleaned")
        detected = email_cleaner.detect_dataframe(df, "text")
        
        cleaned_texts = cleaned["cleaned"].to_list()
        detected_texts = detected["text"].to_list() if len(detected) > 0 else []
        ...
        if detected_texts:
            ...
            assert not any("+44 20 1234 5678" in text for text in detected_texts)
